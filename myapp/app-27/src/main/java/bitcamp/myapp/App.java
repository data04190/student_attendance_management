  ...
  private void loadMember() {
    try {
      ...
      for (int i = 0; i < size; i++) {
        ...
        member.setDate(LocalDate.parse(new String(buf, 0, length, "UTF-8")));

        length = in.read() << 8 | in.read();
        in.read(buf, 0, length);
        member.setStudentName(new String(buf, 0, length, "UTF-8"));

        length = in.read() << 8 | in.read();
        in.read(buf, 0, length);
        member.setEntryTime(LocalTime.parse(new String(buf, 0, length, "UTF-8")));

        length = in.read() << 8 | in.read();
        in.read(buf, 0, length);
        member.setExitTime(LocalTime.parse(new String(buf, 0, length, "UTF-8")));

        length = in.read() << 8 | in.read();
        in.read(buf, 0, length);
        member.setStudyTime(LocalTime.parse(new String(buf, 0, length, "UTF-8")));

        length = in.read() << 8 | in.read();
        in.read(buf, 0, length);
        member.setLateStatus(new String(buf, 0, length, "UTF-8"));
  ...
  private void saveMember() {
    try {
      ...
      for (Member member : memberList) {
        ...
        byte[] bytes = member.getDate().toString().getBytes("UTF-8");
        ...
        bytes = member.getStudentName().getBytes("UTF-8");
        out.write(bytes.length >> 8);
        out.write(bytes.length);
        out.write(bytes);

        bytes = member.getEntryTime().toString().getBytes("UTF-8");
        out.write(bytes.length >> 8);
        out.write(bytes.length);
        out.write(bytes);

        bytes = member.getExitTime().toString().getBytes("UTF-8");
        out.write(bytes.length >> 8);
        out.write(bytes.length);
        out.write(bytes);

        bytes = member.getStudyTime().toString().getBytes("UTF-8");
        out.write(bytes.length >> 8);
        out.write(bytes.length);
        out.write(bytes);

        bytes = member.getLateStatus().getBytes("UTF-8");
        out.write(bytes.length >> 8);
        out.write(bytes.length);
        out.write(bytes);
