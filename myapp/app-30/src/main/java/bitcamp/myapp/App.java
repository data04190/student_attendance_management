  ...
  private void loadMember() {
    try {
      ...
      for (int i = 0; i < size; i++) {
        ...
        member.setDate(LocalDate.parse(in.readUTF()));
        member.setStudentName(in.readUTF());
        member.setEntryTime(LocalTime.parse(in.readUTF()));
        member.setExitTime(LocalTime.parse(in.readUTF()));
        member.setStudyTime(LocalTime.parse(in.readUTF()));
        member.setLateStatus(in.readUTF());
  ...
  private void saveMember() {
    try {
      ...
      for (Member member : memberList) {
        ...
        out.writeUTF(member.getDate().toString());
        out.writeUTF(member.getStudentName());
        out.writeUTF(member.getEntryTime().toString());
        out.writeUTF(member.getExitTime().toString());
        out.writeUTF(member.getStudyTime().toString());
        out.writeUTF(member.getLateStatus());
      }
      out.close();
